...
<div class="container-fluid py-4">
    ...
    <form action="{{ route('admin.collections.update', $collection->id) }}" method="POST" enctype="multipart/form-data" id="collectionForm">
        @csrf
        @method('PUT')
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div id="onedriveLinksContainer">
                            @foreach($collection->oneDriveLinks as $link)
                                <div class="row g-2 mb-2 link-row align-items-center">
                                    <input type="hidden" name="onedrive_links[{{ $link->id }}][id]" value="{{ $link->id }}">
                                    <div class="col-5">
                                        <input type="text" name="onedrive_links[{{ $link->id }}][title]" 
                                            class="form-control form-control-sm" 
                                            value="{{ old('onedrive_links.'.$link->id.'.title', $link->title) }}" 
                                            placeholder="Title (Optional)">
                                    </div>
                                    <div class="col-6">
                                        <input type="url" name="onedrive_links[{{ $link->id }}][url]" 
                                            class="form-control form-control-sm" 
                                            value="{{ old('onedrive_links.'.$link->id.'.url', $link->url) }}" 
                                            placeholder="OneDrive URL" required>
                                    </div>
                        ...
                        <small class="text-muted d-block mt-2">
                            Add or modify OneDrive links for this collection.
                        </small>
                    </div>
                </div>
 ...
 /**
    ...
    function deleteExistingImage(collectionId, index) {
        if (confirm('Are you sure you want to delete this image?')) {
            fetch(`/admin/collections/${collectionId}/images/${index}`, {
                method: 'DELETE',
                headers: {
    ...
    fetch(`/admin/collections/${collectionId}/title-image`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
